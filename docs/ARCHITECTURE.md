...
- **rules/analyzers** apply the heuristics shipped in `promptlint/core.py`.
- **score** ranks by severity.
- **MCP server** (`promptlint mcp`) exposes `scan` for Cognis.Studio agents.
