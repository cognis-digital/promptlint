...
[![PyPI](https://img.shields.io/pypi/v/cognis-promptlint.svg)](https://pypi.org/project/cognis-promptlint/)
[![CI](https://github.com/cognis-digital/promptlint/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/promptlint/actions)
...
PROMPTLINT exists for one job — lint, version, and test prompts as code with a ci gate — and does it without a SaaS bill or heavyweight setup.
...
```bash
promptlint --version
promptlint scan .                      # scan the current project
promptlint scan . --format json
promptlint scan . --fail-on high       # non-zero exit for CI gates
promptlint mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **promptfoo**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`promptlint` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
